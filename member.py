...
def insertMember():
    try:
        cnx = mysql.connector.connect(user='root', password='passroot', host='localhost', database='Library')
        Cursor = cnx.cursor()
...
def deleteMember():
    try:
        ...
        cnx = mysql.connector.connect(user='root', password='passroot', host='localhost', database='Library')
        Cursor = cnx.cursor()
        query = ("SELECT Mno,Mname FROM Member where mno = %s")
        rec_srch = (mno,)
        Cursor.execute(query, rec_srch)
        Rec_count = 0
        for(mno, mname) in Cursor:
            Rec_count += 1
            print("=============================================================")
            print("Name of deleted member")
            print()
